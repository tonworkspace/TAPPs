          ...
          <div className="flex items-center justify-center mb-4">
            <div className="inline-flex p-1 rounded-xl bg-white/5 border border-white/10">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'mining' ? 'bg-blue-500/20 text-blue-300' : 'text-white/60 hover:text-white/80'}`}
              >
                Mining
              </button>
              <button
                onClick={() => setActiveTab('activity')}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${activeTab === 'activity' ? 'bg-blue-500/20 text-blue-300' : 'text-white/60 hover:text-white/80'}`}
              >
                Activity
              </button>
            </div>
            ...
            <div className="mt-4 mb-4 space-y-4">
              {props.isLoadingActivities ? (
                ...
                props.activities.slice(0, 10).map((a) => (
                  <div key={a.id} className="flex items-center gap-3 p-3 bg-white/5 rounded-lg border border-white/10">
                    ...
                    <div className="flex-1">
                      <div className="text-sm text-white capitalize">{a.type.replace(/_/g, ' ')}</div>
                      <div className="text-xs text-white/40">{new Date(a.created_at).toLocaleString()}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-medium text-white">{typeof a.amount === 'number' ? a.amount.toFixed(6) : a.amount} {a.type === 'nova_reward' ? 'NOVA' : 'TON'}</div>
                      <div className="text-xs text-white/40">{a.status}</div>
                    </div>
                  </div>
                ))
              ) : (
