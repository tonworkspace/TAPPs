-- FINAL CORRECTED Withdrawal Approval - Simple SQL Commands
-- All column references fixed to match actual database schema
...
-- =============================================
-- 10. BULK OPERATIONS (Use with caution!)
-- =============================================
...
-- =============================================
-- 11. SIMPLE APPROVAL WORKFLOW
-- =============================================

-- Step-by-step approval process for withdrawal ID 123:
-- 1. Check withdrawal details
SELECT 
    w.id, w.user_id, w.amount, w.status, u.username, u.total_withdrawn
FROM withdrawals w
JOIN users u ON w.user_id = u.id
WHERE w.id = 123;
...
-- 3. Approve the withdrawal (run these 3 commands in sequence)
UPDATE withdrawals SET status = 'COMPLETED', processed_at = NOW() WHERE id = 123;
UPDATE users SET total_withdrawn = total_withdrawn - (SELECT amount FROM withdrawals WHERE id = 123), total_payout = total_payout + (SELECT amount FROM withdrawals WHERE id = 123) WHERE id = (SELECT user_id FROM withdrawals WHERE id = 123);
INSERT INTO activities (user_id, type, amount, status, created_at) SELECT user_id, 'withdrawal', amount, 'COMPLETED', NOW() FROM withdrawals WHERE id = 123;

-- 4. Verify the approval
SELECT 
    w.id, w.status, w.processed_at, u.total_withdrawn, u.total_payout
FROM withdrawals w
JOIN users u ON w.user_id = u.id
WHERE w.id = 123;
